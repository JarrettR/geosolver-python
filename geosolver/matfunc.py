...
class Table(list):
    ...
    def map(self, op, rhs=None ):
        ...
        the larger (i.e. match a scalar to every element in a vector or
...
class Vec(Table):
    ...
    def normSquared(self):
        return abs( self.dot(self.conjugate()))
    ...
    def house( self, index ):
        """Compute a Householder vector which zeroes all but the index
        element after a reflection"""
...
if __name__ == '__main__':
    ...
    points = (xvec, yvec) = funToVec(lambda x: math.sin(x)+2*math.cos(.7*x+.1),
                                     low=0,
                                     high=3,
                                     EqualSpacing=1)
